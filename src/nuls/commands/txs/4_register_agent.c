...
static void uiProcessor_4_register_agent(uint8_t step) {
  ...
  switch (step) {
    ...
    case 6:
      //Commission Rate
      snprintf(lineBuffer, 50, "%f", txContext.tx_fields.register_agent.commissionRate);
      break;
    case 7:
      //Fees
...
void tx_parse_specific_4_register_agent() {
  ...
  switch(txContext.tx_parsing_state) {
    ...
    case _4_REGISTER_AGENT_COMMISSION_RATE:
      ...
      is_available_to_parse(AMOUNT_LENGTH);
      //It's a Double LE...
      os_memmove(&txContext.tx_fields.register_agent.commissionRate, txContext.bufferPointer, AMOUNT_LENGTH)
      transaction_offset_increase(AMOUNT_LENGTH);
      PRINTF("commissionRate: %f\n", txContext.tx_fields.register_agent.commissionRate);
