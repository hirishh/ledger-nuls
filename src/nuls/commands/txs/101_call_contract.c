...
static void uiProcessor_101_call_contract(uint8_t step) {
  ...
  tx_type_specific_101_call_contract_t *cc = &(txContext.tx_fields.call_contract);
...
void tx_finalize_101_call_contract() {
  ...
  PRINTF("tx_finalize_101_call_contract - A\n");

  /*
  PRINTF("tx_finalize_101_call_contract - nOut %d\n", txContext.nOut);
  PRINTF("tx_finalize_101_call_contract - txContext.outputAddress %.*H\n", ADDRESS_LENGTH, txContext.outputAddress[0]);
  PRINTF("tx_finalize_101_call_contract - BLACK_HOLE_ADDRESS %.*H\n", ADDRESS_LENGTH, BLACK_HOLE_ADDRESS);
  PRINTF("tx_finalize_101_call_contract - txContext.outputAmount %.*H\n", AMOUNT_LENGTH, txContext.outputAmount[0]);
  PRINTF("tx_finalize_101_call_contract - BLACK_HOLE_ALIAS_AMOUNT %.*H\n", AMOUNT_LENGTH, BLACK_HOLE_ALIAS_AMOUNT);
  PRINTF("tx_finalize_101_call_contract - nuls_secure_memcmp address %d\n", nuls_secure_memcmp(txContext.outputAddress[0], BLACK_HOLE_ADDRESS, ADDRESS_LENGTH));
  PRINTF("tx_finalize_101_call_contract - nuls_secure_memcmp amount %d\n", nuls_secure_memcmp(txContext.outputAmount[0], BLACK_HOLE_ALIAS_AMOUNT, AMOUNT_LENGTH));
  */

  PRINTF("tx_finalize_101_call_contract - B\n");
  ...
  PRINTF("tx_finalize_101_call_contract - C\n");
